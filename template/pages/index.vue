  ...
  <div>
    <div v-if="jobSearchStore.results && jobSearchStore.results.length > 0">
      ...
      <div v-for="job in jobSearchStore.results" :key="job.guid" :ref="setJobRef">
        <nuxt-link :to="`${buildJobDetailUrl(job)}`" :id="`job-link-${job.guid}`" @click="handleJobClick(job)">
          {{ job.title_exact }}
        </nuxt-link>
      </div>
      <button v-if="jobSearchStore.page < jobSearchStore.totalPages" @click="loadMore"
